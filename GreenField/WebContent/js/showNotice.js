function previewNotice(id) {
    ...
    //TODO pass id in json format to the webservice
    //TODO remove if clause for lazy loading
...
function processImageData(dataResponse) {
    ...
    console.log("previewNoticeWebservice Response" + dataResponse.notice.id);

    /*var len = dataResponse.notice.length;
...
*/
    ...
    //if (len > 10) {
       // for (var i = 0; i < 10; i++) {
            data = {
                "id": dataResponse.notice.id,
                "noticeName": dataResponse.notice.noticeName,
                "publishedBy": dataResponse.notice.publishedBy,
                "category": dataResponse.notice.category,
                "dateAdded": dataResponse.notice.dateAdded,
                "image":dataResponse.notice.image
            };
       // }
        ...
        setTimeout(function () {
            ...
            for (var j = 0, i = 10; i < len; j++, i++) {
                ...
                data1[j] = {
                    ...
                    "image":dataResponse.notices[i].image
                };
            }

            updateNoticeImage(data1);
            $("#loadinggif").hide();
        }, 1);
    } else {
        for (var i = 0; i < len; i++) {
...
updateNoticeImage = function (result) {
 //document.getElementById(noticeContent).src="data:image/png;base64,"+result.image;
 var a = document.getElementById("noticeContent");
 a.src="data:image/png;base64,"+result.image;
	$.mobile.changePage("#gfPreviewNotice");
};

function changePageReq_ShowNoticeContent(id)
{
	previewNotice(id);
};
